...
library LibZOCSA {
   ...
   */
  event ZOCSAUserRewardWithdraw(
    address indexed ZOCSAToken,
    address erc20Token,
  ...
  */
  ...
  function mint(address token, address from, address to, uint256 amount) internal {
    ...
    require(LibWhitelist.isWhitelisted(to) > 0, "ZOCSA: Recipient not whitelisted !");
    require(
      (amount + t.totalSupply) <= t.maxSupply,
      "ZOCSA: Cannot mint new OCSA, max supply reached"
    );

    _calculateDividend(token, to);
    if (s.diamondAdmins[from] == false) {
      bool success = IERC20(t.rewardToken).transferFrom(from, s.treasury, (t.tokenPrice * amount));
      if (success == false) {
  ...
  function boundUserOCSA(address token, address user, uint256 amount) internal {
    ...
    require(LibWhitelist.isWhitelisted(user) > 0, "ZOCSA: User not whitelisted !");
    require(
      t.unboundedOcsas[user] >= amount,
      "ZOCSA: Not enought unbounded OCSA to bound, consulte available balance before retrying"
    );
  ...
  */
  ...
  function withdrawUserReward(
    ...
    require(LibWhitelist.isWhitelisted(from) > 0, "ZOCSA: User not whitelisted !");
    ZOCSAToken storage t = LibAppStorage.diamondStorage().zOcsas[token];
   ...
   * @return bool Returns true if successful.
   */
  function dispatchProjectReward(
    address token,
    address from,
    uint256 amount
  ) internal returns (bool) {
  ...
  function getAllCollectionsInfos() internal view returns (ZOCSAInfos[] memory) {
    AppStorage storage s = LibAppStorage.diamondStorage();
    address[] memory collections = s.zOcsaCollections;
    uint256 length = collections.length;
    ZOCSAInfos[] memory data = new ZOCSAInfos[](length);

    for (uint256 i = 0; i < length; i++) {
      ZOCSAToken storage t = s.zOcsas[collections[i]];
  ...
  function getUserInfo(
    ...
    address user
  ) internal view returns (ZOCSAUserInfo memory) {
    AppStorage storage s = LibAppStorage.diamondStorage();
    ZOCSAToken storage t = s.zOcsas[collectionAddress];
    uint256 whitelistStatus = s.isWhitelisted[user];
   ...
   */
  function _depositRewardNoRemainder(
    address token,
    address from,
    uint256 amount,
    uint256 totalSupply,
    uint256 unboundedOcsaSupply,
    uint256 boundedOcsaSupply,
    uint256 maxSupply
  ) internal {
    ZOCSAToken storage t = LibAppStorage.diamondStorage().zOcsas[token];
    ...
    if (adjustedAmount > 0) {
      ...
      require(
        IERC20(t.rewardToken).transferFrom(from, address(this), adjustedAmount),
        "ZOCSA: Reward Deposit Transfer failed"
      );
    }
   ...
   * @param maxSupply The maximum possible supply of the ZOCSA token.
   */
  function _depositRewardWithRemainder(
    address token,
    address from,
    uint256 amount,
    uint256 totalSupply,
    ...
    t.dividends[address(this)] += amount;
    require(
      IERC20(t.rewardToken).transferFrom(from, address(this), amount),
      "ZOCSA: Reward Deposit Transfer failed"
    );
